...
function getFileContents(filePath) {
  return new Promise((resolve, reject) => {
    fs.readFile(filePath, {encoding: 'utf8', flag: 'w+'}, (err, fileData) => {
      if (err) {
        return reject(err);
      }
  ...
  const prompts = [
    {
      ...
      default: currentEnv.BACKEND_URL || 'http://0.0.0.0:3030'
    }, {
      type: 'input',
      name: 'HOST',
      ...
      default: currentEnv.PORT || 3000
    }, {
      type: 'input',
      name: 'IMAGE_URL_DOMAIN',
      message: 'The image server domain (including protocol).',
      default: currentEnv.IMAGE_URL_DOMAIN || ''
    }, {
      type: 'input',
      name: 'ROLLBAR_TOKEN',
      message: 'The Rollbar client token.',
      default: currentEnv.ROLLBAR_TOKEN || ''
    }
  ];
